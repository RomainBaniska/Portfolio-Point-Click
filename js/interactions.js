...
toileScreenProject2.addEventListener("click", async () => {
    ...
    toileScreenProject3.addEventListener("click", () => {
            // Crée et affiche la bulle d’information
    const bulleConstruction = new PIXI.Text({
        text: "Désolé ce projet est encore en construction",
        style: dialogueStyleLong
    });
    bulleConstruction.anchor.set(0.5, 0);
    bulleConstruction.x = toileScreen.x + (toileScreen.width * 0.53);
    bulleConstruction.y = toileScreen.y + (toileScreen.height * 0.05);
    bulleConstruction.zIndex = 12;
    screenBackgroundContainer.addChild(bulleConstruction);

    // Retire la bulle et remet le clone à l'arrêt après quelques secondes
    setTimeout(() => {
        screenBackgroundContainer.removeChild(bulleConstruction);
        guybrushClone.gotoAndStop(0);
    }, 4000);
        });
